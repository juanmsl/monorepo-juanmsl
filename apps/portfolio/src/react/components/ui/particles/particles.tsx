...
export const Particles = ({
  ...
  const drawCircle = useCallback(
    (circle: Circle, update = false) => {
      if (context.current) {
        ...
        context.current.fillStyle = `${theme.colors.text.main}${_alpha}`;
  ...
  const onMouseMove = useCallback(() => {
    if (canvasRef.current) {
      ...
      const _x = x - rect.left - w / 2;
      const _y = y - rect.top - h / 2;
      const inside = _x < w / 2 && _x > -w / 2 && _y < h / 2 && _y > -h / 2;
